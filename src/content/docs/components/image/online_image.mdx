...
The `online_image` platform of the [image](/components/image/) component allows you to define images that will be
...
This component has a dependency to [Http Request](/components/http_request/); the configuration options you set to the `http_request` component will also apply here.
...
- **placeholder** (*Optional*, [ID](/guides/configuration-types#id)): ID of another [image](/components/image/) to display while the downloaded image is not yet ready.
...
- **transparency** (*Optional*): If set the alpha channel of the input image will be taken into account. The possible values are `opaque` (default), `chroma_key` and `alpha_channel`. See the discussion on transparency in the [image component](/components/image#transparency-options).
...
- **update_interval** (*Optional*, [Time](/guides/configuration-types#time)): Redownload the image when the specified time has elapsed. Defaults to `never` (i.e. the update component action needs to be called manually).
...
- Add a `component.update <image_id>` in the `on_connect:` action on the [Wifi](/components/wifi/) component.
- Explicitly set an `update_interval`.
- Call `component.update <image_id>` in an [Interval](/components/interval/) block.
...
- [Animation Component](/components/image/animation/)
- <APIRef text="online_image.h" path="online_image/online_image.h" />
