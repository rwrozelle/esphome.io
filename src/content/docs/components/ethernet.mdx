...
- **ESP32**: Supports RMII PHY chips (LAN8720, RTL8201, etc.) and SPI Ethernet chips (W5500, DM9051, CH390, ENC28J60).
- **RP2040/RP2350**: Supports SPI/PIO Ethernet chips (W5100, W5500, W6100, W6300, ENC28J60).

On ESP32 family devices, Ethernet may be configured alongside [WiFi](/components/wifi/) in the same configuration;
this requires a `network:` `priority:` list naming both interfaces — see
[Multi-Interface Support](/components/network/#multi-interface-support) on the [Network](/components/network/)
component. Using more than one network interface is not supported on other platforms.
...
> [!NOTE]
> The WCH CH390 shares the Davicom DM9051 register map but reports its own vendor and product IDs, so the
> `DM9051` type rejects it during initialization with a `wrong Vendor ID` error. Use `CH390`.
...
- [Ethernet Info Text Sensor](/components/text_sensor/ethernet_info/)
- [ESP32 Ethernet PHY connection info](https://pcbartists.com/design/embedded/esp32-ethernet-phy-schematic-design/)
- <APIRef text="ethernet_component.h" path="ethernet/ethernet_component.h" />
