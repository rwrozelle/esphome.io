...
- **response_size** (*Optional*, int): Number of bytes the poll reads to cover this binary sensor. For the register types (`holding`/`read`) it defaults to `2` (one register); raise it only when the bit you are reading sits in a wider response, such as a `custom_command` reply spanning several registers. It sets the size of the read, not which bit is published — the bit is always selected by `offset` and `bitmask`. It has no effect on the `coil` and `discrete_input` types, which are always read as a single bit.
