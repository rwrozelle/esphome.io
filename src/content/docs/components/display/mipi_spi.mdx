...
- **rotation** (*Optional*): Rotate the display presentation. Choose one of `0°`, `90°`, `180°`, or `270°`. If the driver chip supports hardware rotation for the given orientation this will be translated to the appropriate hardware command. If hardware rotation is not supported, the display will be rotated in software.
...
- **data_rate** (*Optional*): The SPI data rate. Defaults to 10MHz but board presets may override this.
  The maximum achievable data rate will depend on the chip type (e.g. ESP32 vs ESP32-S3), the pins used (on ESP32 using the default SPI pins allows higher rates),
  and the connection type (on-board connections will support higher rates than long cables or DuPont wires.) If in doubt, start with a low speed
  and test higher rates to find what works.
...
- **transform** (*Optional*): Can be used to mirror and/or swap the x and y axes. Should not be required unless using the `CUSTOM` model.
  When selecting a predefined model, simply use `rotation` instead to orient the display, for example to switch from portrait to landscape.
  If this option is specified, then the `dimensions` option must also be provided.
  The value can either be the string `disabled` to disable hardware transform, or a dictionary with the options listed below.
  For the `CUSTOM` model, use `transform: disabled` if the display does not support axis swapping, which will prevent a `rotation`
  being translated to a hardware transform.
