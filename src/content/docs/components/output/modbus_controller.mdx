...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)):
  Lambda is evaluated before the modbus write command is created. The value is passed in as `float x` (or `bool x` for `register_type: coil`) and an empty payload vector whose element type depends on `register_type` (see below).
